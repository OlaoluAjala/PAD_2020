...
void setup()  {
  ...
  g.addNonNeighbor(0x405D9805);  // node 5
  g.addNonNeighbor(0x40498065);  // node 4
  // manually save lambdas
  g.getVertexByUniqueID(1)->setLambdaMin(15.8*base);
  g.getVertexByUniqueID(1)->setLambdaMax(29*base);
  g.getVertexByUniqueID(2)->setLambdaMin(3.2*base);
  g.getVertexByUniqueID(2)->setLambdaMax(7.5*base);
...
void loop() {
  if(de == false) {
    ...
    if(a.isSynced()) {
      ...
      //long t = a.nonleaderOptimalDispatch(1.15*base,cPin);
